...
pub const RefList = struct {
    ...
    pub fn init(
        ...
        switch (repo_kind) {
            .git => {
                ...
                defer path.deinit();
                try ref_list.addRefs(hash_kind, state, heads_dir, &path);
            },
            .xit => {
                if (try state.extra.moment.cursor.readPath(void, &.{
                    .{ .hash_map_get = .{ .value = hash.hashInt(hash_kind, "refs") } },
                    .{ .hash_map_get = .{ .value = hash.hashInt(hash_kind, "heads") } },
                })) |heads_cursor| {
    ...
    fn addRefs(
        self: *RefList,
        comptime hash_kind: hash.HashKind,
        state: rp.Repo(.git, hash_kind).State(.read_only),
        dir: std.fs.Dir,
        ...
        while (try iter.next()) |entry| {
            ...
            switch (entry.kind) {
                ...
                .directory => {
                    ...
                    try self.addRefs(hash_kind, state, next_dir, &next_path);
                },
                else => {},
            }
        }
...
pub fn readRecur(
    ...
    switch (input) {
        .ref => |ref| {
            ...
            const content = read(repo_kind, hash_kind, state, ref_path, &read_buffer) catch |err| switch (err) {
                error.RefNotFound => return null,
                else => |e| return e,
            };

            if (RefOrOid(hash_kind).initFromDb(content)) |next_input| {
                return try readRecur(repo_kind, hash_kind, state, next_input);
            } else {
                return null;
            }
        },
    ...
    switch (repo_kind) {
        ...
        .xit => {
            ...
            if (Ref.initFromPath(ref_path)) |ref| {
                const refs_cursor = (try map.getCursor(hash.hashInt(hash_kind, "refs"))) orelse return error.RefNotFound;
                const refs = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_only).init(refs_cursor);
                switch (ref.kind) {
                    .local => {
                        const heads_cursor = (try refs.getCursor(hash.hashInt(hash_kind, "heads"))) orelse return error.RefNotFound;
                        map = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_only).init(heads_cursor);
                    },
                    .remote => |remote| {
                        const remotes_cursor = (try refs.getCursor(hash.hashInt(hash_kind, "remotes"))) orelse return error.RefNotFound;
                        const remotes = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_only).init(remotes_cursor);
                        const remote_cursor = (try remotes.getCursor(hash.hashInt(hash_kind, remote))) orelse return error.RefNotFound;
                        map = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_only).init(remote_cursor);
                    },
                }
                ref_name = ref.name;
            }

            const ref_cursor = (try map.getCursor(hash.hashInt(hash_kind, ref_name))) orelse return error.RefNotFound;
...
pub fn readHeadMaybe(
    ...
    const content = try read(repo_kind, hash_kind, state, "HEAD", &buffer);
    if (RefOrOid(hash_kind).initFromDb(content)) |input| {
        return try readRecur(repo_kind, hash_kind, state, input);
    } else {
        return null;
    }
}
...
pub fn readHead(
    ...
    if (try readHeadMaybe(repo_kind, hash_kind, state)) |buffer| {
        return buffer;
    } else {
...
pub fn readHeadNameAlloc(
    ...
    var buffer = [_]u8{0} ** MAX_REF_CONTENT_SIZE;
    return try allocator.dupe(u8, try readHeadName(repo_kind, hash_kind, state, &buffer));
}
...
pub fn write(
    ...
    switch (repo_kind) {
        ...
        .xit => {
            ...
            if (Ref.initFromPath(ref_path)) |ref| {
                const refs_cursor = try map.putCursor(hash.hashInt(hash_kind, "refs"));
                const refs = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_write).init(refs_cursor);
                switch (ref.kind) {
                    .local => {
                        const heads_cursor = try refs.putCursor(hash.hashInt(hash_kind, "heads"));
                        map = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_write).init(heads_cursor);
                    },
                    .remote => |remote_name| {
                        const remotes_cursor = try refs.putCursor(hash.hashInt(hash_kind, "remotes"));
                        const remotes = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_write).init(remotes_cursor);
                        const remote_cursor = try remotes.putCursor(hash.hashInt(hash_kind, remote_name));
                        map = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_write).init(remote_cursor);
                    },
                }
                ref_name = ref.name;
            }

            const ref_name_hash = hash.hashInt(hash_kind, ref_name);
            const ref_name_set_cursor = try state.extra.moment.putCursor(hash.hashInt(hash_kind, "ref-name-set"));
            const ref_name_set = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_write).init(ref_name_set_cursor);
            ...
            const ref_content_set_cursor = try state.extra.moment.putCursor(hash.hashInt(hash_kind, "ref-content-set"));
            const ref_content_set = try rp.Repo(repo_kind, hash_kind).DB.HashMap(.read_write).init(ref_content_set_cursor);
            var ref_content_cursor = try ref_content_set.putKeyCursor(hash.hashInt(hash_kind, content));
...
pub fn writeRecur(
    ...
    var buffer = [_]u8{0} ** MAX_REF_CONTENT_SIZE;
    const existing_content = read(repo_kind, hash_kind, state.readOnly(), ref_path, &buffer) catch |err| switch (err) {
        error.RefNotFound => {
            try write(repo_kind, hash_kind, state, ref_path, oid_hex);
            return;
        },
        else => |e| return e,
    };
    if (RefOrOid(hash_kind).initFromDb(existing_content)) |input| {
        switch (input) {
            .ref => |ref| {
                var ref_path_buffer = [_]u8{0} ** MAX_REF_CONTENT_SIZE;
                const next_ref_path = try ref.toPath(&ref_path_buffer);
                try writeRecur(repo_kind, hash_kind, state, next_ref_path, oid_hex);
            },
            .oid => try write(repo_kind, hash_kind, state, ref_path, oid_hex),
        }
    } else {
        try write(repo_kind, hash_kind, state, ref_path, oid_hex);
    }
...
pub fn writeHead(
    ...
    state: rp.Repo(repo_kind, hash_kind).State(.read_write),
    target: []const u8,
    oid_hex_maybe: ?[hash.hexLen(hash_kind)]u8,
) !void {
    var buffer = [_]u8{0} ** MAX_REF_CONTENT_SIZE;
    const content =
        // target is a ref, so make HEAD point to it
        if (try readRecur(repo_kind, hash_kind, state.readOnly(), .{ .ref = .{ .kind = .local, .name = target } }) != null)
        ...
        try std.fmt.bufPrint(&buffer, "ref: refs/heads/{s}", .{target});
    try write(repo_kind, hash_kind, state, "HEAD", content);
}
