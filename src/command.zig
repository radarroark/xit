...
pub fn Command(comptime hash_kind: hash.HashKind) type {
    return union(enum) {
        ...
        pub fn init(allocator: std.mem.Allocator, args: []const []const u8) !Command(hash_kind) {
            ...
            if (std.mem.eql(u8, sub_command, "init")) {
                ...
                if (map_args.contains("help")) {
                    ...
                    const diff_opts: df.BasicDiffOptions(hash_kind) = if (map_args.contains("--staged"))
                        .index
                    else
...
test "command" {
    ...
    {
        var command_or_err = Command(hash_kind).init(allocator, &.{ "add", "--cli" });
    ...
    {
        var command = try Command(hash_kind).init(allocator, &.{ "add", "file.txt" });
        defer command.deinit();
        try std.testing.expect(command == .cli and command.cli.? == .add);
    }

    {
        var command_or_err = Command(hash_kind).init(allocator, &.{ "commit", "-m" });
        if (command_or_err) |*command| {
            defer command.deinit();
            return error.ExpectedError;
        } else |err| {
    ...
    {
        var command = try Command(hash_kind).init(allocator, &.{ "commit", "-m", "let there be light" });
        defer command.deinit();
