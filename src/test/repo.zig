...
fn testSimple(comptime repo_kind: rp.RepoKind, comptime repo_opts: rp.RepoOpts(repo_kind)) !void {
    ...
    {
        var result = try repo.switchMount(allocator, .{ .action = .update, .ref_or_oid = .{ .oid = &commit_b } });
        defer result.deinit();
    }
    ...
    {
        var result = try repo.switchMount(allocator, .{ .action = .update, .ref_or_oid = .{ .oid = &commit_a } });
        defer result.deinit();
    }
    ...
    {
        var result = try repo.switchMount(allocator, .{ .action = .update, .ref_or_oid = .{ .oid = &commit_c } });
        defer result.deinit();
    }
...
fn testMerge(comptime repo_kind: rp.RepoKind, comptime repo_opts: rp.RepoOpts(repo_kind)) !void {
    ...
    {
        var result = try repo.switchMount(allocator, .{ .action = .replace, .ref_or_oid = .{ .ref = .{ .kind = .head, .name = "foo" } } });
        defer result.deinit();
    }
    ...
    {
        var result = try repo.switchMount(allocator, .{ .action = .replace, .ref_or_oid = .{ .ref = .{ .kind = .head, .name = "bar" } } });
        defer result.deinit();
    }
    ...
    {
        var result = try repo.switchMount(allocator, .{ .action = .replace, .ref_or_oid = .{ .ref = .{ .kind = .head, .name = "master" } } });
        defer result.deinit();
    }
    ...
    {
        var switch_result = try repo.switchMount(allocator, .{ .action = .replace, .ref_or_oid = .{ .ref = .{ .kind = .head, .name = "foo" } } });
