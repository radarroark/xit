...
fn testFetchLarge(
    ...
    // copy current git repo into server dir
    {
        var src_repo_dir = try cwd.openDir(".git", .{ .iterate = true });
        defer src_repo_dir.close();

        try server_dir.makeDir(".git");

        var dest_repo_dir = try server_dir.openDir(".git", .{});
    ...
    // open server repo
    var server_repo = try rp.Repo(.git, .{ .is_test = true }).open(allocator, .{ .cwd = server_dir });
    defer server_repo.deinit();
    ...
    // for now, we're requesting an old commit to make it go faster
    // TODO: change to 12cc0cb7856d667a10cb37a61e43346a1e60ae46
    // once iterating over packfiles is faster
    const refspecs = &.{
        "+46d3bcd84c9851b2b937d5a0f89a15e4f7842564:refs/heads/master",
    };
    ...
    // make sure fetch was successful
    {
        const build_zig = try temp_dir.openFile("client/build.zig", .{});
...
fn testPushLarge(
    ...
    // copy current git repo into client dir
    {
        var src_repo_dir = try cwd.openDir(".git", .{ .iterate = true });
        defer src_repo_dir.close();

        try client_dir.makeDir(".git");

        var dest_repo_dir = try client_dir.openDir(".git", .{});
        defer dest_repo_dir.close();
    ...
    // open client repo
    var client_repo = try rp.Repo(repo_kind, repo_opts).open(allocator, .{ .cwd = client_dir });
    defer client_repo.deinit();

    // for now, we're resetting to an old commit to make it go faster
    // TODO: change to 12cc0cb7856d667a10cb37a61e43346a1e60ae46
    // once writing packfiles is faster
    {
        var result = try client_repo.resetDir(allocator, .{ .target = .{ .oid = "46d3bcd84c9851b2b937d5a0f89a15e4f7842564" }, .force = true });
        defer result.deinit();
    }
    ...
    // make sure push was successful
    {
        const server_oid_master = (try server_repo.readRef(.{ .kind = .head, .name = "master" })).?;
        const client_oid_master = (try client_repo.readRef(.{ .kind = .head, .name = "master" })).?;
        try std.testing.expectEqualStrings(&server_oid_master, &client_oid_master);
    }
}
