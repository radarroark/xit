...
pub fn applyPatch(
    ...
    while (true) {
        ...
        switch (try std.meta.intToEnum(ChangeKind, change_kind)) {
            .new_edge => {
                ...
                const line_id_bytes = try hash.numToBytes(LineId(repo_opts.hash).Int, line_id_int);
                ...
                {
                    var parent_line_id_bytes = try hash.numToBytes(LineId(repo_opts.hash).Int, parent_line_id_int);
    ...
    while (true) {
        const current_line_id_bytes = try hash.numToBytes(LineId(repo_opts.hash).Int, current_line_id_int);
