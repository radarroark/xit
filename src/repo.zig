        ...
        pub fn pull(self: *Repo(repo_kind, repo_opts), allocator: std.mem.Allocator, remote_name: []const u8, remote_ref_name: []const u8) !PullResult {
            ...
            switch (repo_kind) {
                .git => {
                    ...
                    var merge_result = try mrg.Merge(repo_kind, repo_opts).init(.{ .core = &self.core, .extra = .{} }, allocator, .{ .kind = .merge, .action = .{ .new = .{ .source = .{ .ref = remote_ref } } } });
                ...
                .xit => {
                    ...
                    const Ctx = struct {
                        ...
                        pub fn run(ctx: @This(), cursor: *DB.Cursor(.read_write)) !void {
                            ...
                            var merge_result = try mrg.Merge(repo_kind, repo_opts).init(state, ctx.allocator, .{ .kind = .merge, .action = .{ .new = .{ .source = .{ .ref = remote_ref } } } });
                            errdefer merge_result.deinit();
