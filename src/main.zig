    ...
    switch (try cmd.CommandDispatch(repo_kind, repo_opts.hash).init(&cmd_args)) {
        ...
        .cli => |cli_cmd| switch (cli_cmd) {
            ...
            .clone => |clone_cmd| {
                ...
                // if no hash was specified, use AnyRepo to detect the hash being used
                const repo_opts_with_ctx = repo_opts.withProgressCtx(ProgressCtx);
                var any_repo = try rp.AnyRepo(repo_kind, repo_opts_with_ctx).open(io, allocator, .{ .path = cwd_path });
                defer any_repo.deinit(allocator);
                switch (any_repo) {
                    .none => return error.HashKindNotFound,
                    inline else => |*repo| {
                ...
                const repo_opts_with_ctx = repo_opts.withProgressCtx(ProgressCtx);
                var repo = try rp.Repo(repo_kind, repo_opts_with_ctx).open(io, allocator, .{ .path = cwd_path });
                defer repo.deinit(allocator);
                try runCommand(repo_kind, repo_opts_with_ctx, &repo, io, allocator, cli_cmd, writers);
            },
        },
    }
}
        ...
        .patch => |patch_cmd| switch (repo_kind) {
            ...
            .xit => switch (patch_cmd) {
                ...
                .all => {
                    var clear_line = false;
                    var progress_node: ?std.Progress.Node = null;
                    try repo.patchAll(io, allocator, .{ .writers = writers, .clear_line = &clear_line, .node = &progress_node });
                },
            },
        },
    }
...
pub fn main() !u8 {
    ...
    runPrint(.xit, .{}, io, allocator, args.items, cwd_path, writers) catch |err| switch (err) {
