...
* Currently, when cloning or fetching, patches are generated for every commit object that is received. For this reason, it will take a lot longer than it does with git. My current plan is to do patch generation as an optional extra step, rather than forcing you to do it during the clone. If you choose not to generate patches, xit will use a three-way merge for merging and cherry-picking, just like git. That way, you will have to opt-in to better merging if you want it.
