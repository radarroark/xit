...
* Currently, patch-based merging must be explicitly enabled with `xit patch on`. This is because it can take a long time on repos with large histories, since it has to generate a patch for every commit in the entire history. If you leave it disabled, xit will fall back on the three-way merge for merging and cherry-picking, just like git.
